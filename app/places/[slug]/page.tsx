...
export async function generateMetadata({ params }: PlaceDetailPageProps): Promise<Metadata> {
  ...
  const description = place
    ? `${place.name_zh}：釜山${getPlaceCategoryLabel(place.category, "zh")}，${featureText ? `${featureText}。` : ""}${chinaSummary.summary}`
    : "釜山广安里地点详情。";
  ...
  return (
    <main className="safe-bottom mx-auto max-w-3xl px-4 pb-6 pt-4">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-base text-slate-500">韩文原名 · {place.name_ko}</p>
            </div>
            <SaveButton
              className="h-11 px-3"
              ...
              item={{
                ...
                href: `/places/${place.slug}`,
                imageUrl: trustedImageUrl,
                meta: [getPlaceCategoryLabel(place.category, "zh"), placeHasCoordinates ? walkingText : ""].filter(Boolean).join(" · "),
              }}
            />
          </div>
          <div className="mt-4">
      ...
      {place.category === "restaurant" ? (
        ...
        <section className="mt-6 space-y-3">
          ...
          <div className="rounded-[24px] bg-teal-700 p-5 text-white shadow-sm">
            ...
            <p className="mt-3 text-xs font-bold text-teal-100">给店员看的韩语原文</p>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2">
        <InfoPanel icon={Users} title="等候" body={place.waiting_info_zh} />
        <InfoPanel
          icon={MapPin}
          title="怎么去？"
          body={directionsText}
...
function InfoPanel({
  ...
  return (
    <div className="rounded-[24px] bg-white p-5 shadow-sm ring-1 ring-slate-200">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-700">{body || "信息确认中"}</p>
    </div>
  );
}
