...
const fieldOptions: Array<{ value: PlaceCorrectionField; labels: LocalizedText; placeholder: LocalizedText }> = [
  ...
  { value: "location", labels: { zh: "位置/地址", en: "Location/address", ja: "場所・住所", ko: "위치/주소" }, placeholder: { zh: "请填写正确位置或地址", en: "Enter the correct location or address", ja: "正しい場所または住所を入力", ko: "정확한 위치나 주소를 입력해 주세요" } },
...
const correctionCopy: Record<Locale, {
  trigger: string; title: string; description: string; loginTitle: string; loginDescription: string; field: string; current: string;
  ...
  zh: { trigger: "信息有误", title: "补充或修改商家信息", description: "选择停业、营业时间、价格、菜单、位置或其他原因。管理员确认后才会更新。", loginTitle: "信息有误吗？", loginDescription: "登录后可提交地点信息修改。", field: "需要修改的信息", current: "当前登记信息", suggestion: "新的信息", source: "来源链接（可选）", sourceHelp: "官方主页、地图、菜单页面等", notes: "补充说明（可选）", notesPlaceholder: "请补充确认日期或其他说明", submit: "提交信息", submitting: "提交中…", success: "已提交，管理员确认后会更新。", loading: "加载中…" },
  en: { trigger: "Report incorrect info", title: "Update business information", description: "Choose closure, hours, price, menu, location, or another reason. Changes require admin review.", loginTitle: "Is something incorrect?", loginDescription: "Log in to submit a place information update.", field: "Information to update", current: "Current information", suggestion: "New information", source: "Source URL (optional)", sourceHelp: "Official site, map page, menu page, etc.", notes: "Additional note (optional)", notesPlaceholder: "Add the verification date or context", submit: "Submit update", submitting: "Submitting…", success: "Submitted for administrator review.", loading: "Loading…" },
  ja: { trigger: "情報が違う", title: "店舗情報を修正・追加", description: "閉店、営業時間、価格、メニュー、場所、その他から選択してください。管理者確認後に更新されます。", loginTitle: "情報に誤りがありますか？", loginDescription: "ログイン後、店舗情報の修正を送信できます。", field: "修正する情報", current: "現在の登録情報", suggestion: "新しい情報", source: "出典URL（任意）", sourceHelp: "公式サイト、地図、メニューページなど", notes: "補足（任意）", notesPlaceholder: "確認日や補足情報を入力", submit: "情報を送信", submitting: "送信中…", success: "送信しました。管理者確認後に更新されます。", loading: "読み込み中…" },
  ...
  if (!user) {
    ...
    return (
      <details id="place-correction" className="group mt-6 scroll-mt-28">
        ...
        <div className="mt-3"><AuthRequiredPanel title={copy.loginTitle} description={copy.loginDescription} locale={locale} /></div>
      </details>
    );
  }
...
function CorrectionTrigger({ label }: { label: string }) {
  return (
    <summary className="inline-flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-full bg-white px-4 text-sm font-black text-slate-700 shadow-sm ring-1 ring-slate-200">
      <MessageSquareWarning size={17} className="text-amber-600" aria-hidden="true" />
      {label}
    </summary>
  );
}
